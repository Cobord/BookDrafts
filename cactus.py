...
class CactusGroup:
    ...
    def to_permutation(self) -> List[int]:
        ...
        zero_indexed_answer = reduce(perm_multiply_0,(to_permutation_helper(z) for z in self.element),list(range(0,self.index)))
        ...
        for q in range(p+1,MY_N+1):
            ...
            for k in range(q+1,MY_N):
                for l in range(k+1,MY_N+1):
                    ...
                    assert perm_multiply(pi_pq,pi_kl) == perm_multiply(pi_kl,pi_pq)
